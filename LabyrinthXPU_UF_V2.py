...
class EncryptionHandler(FileSystemEventHandler):
    def __init__(self, key, trigger, mode, directory):
        super().__init__()
        self.key = key
    ...
    # Handle encryption for individual or all files
...
class DecryptionHandler(FileSystemEventHandler):
    ...
    # Handle decryption for individual or all files
    def handle_file(self, file_path):
        if self.mode == "Individual" or (self.mode == "Group" and self.is_group(file_path)):
...
class EncryptionApp:
    def __init__(self, master):
        ...
        self.encrypt_trigger_menu = tk.OptionMenu(master, self.encrypt_trigger, "Create", "Delete", "Modify", command=self.update_mode)
        ...
        self.encrypt_mode_menu = tk.OptionMenu(master, self.encrypt_mode, "Individual", "Group", "All", command=self.update_mode)
    ...
    # Method to update mode based on user selection
    def update_mode(self, event=None):
        if hasattr(self, 'handler'):
            self.handler.mode = self.encrypt_mode.get()

    # Method to start monitoring
    def start_monitoring(self):
        if hasattr(self, 'directory') and hasattr(self, 'key_file'):
            self.handler = EncryptionHandler(self.load_key(), self.encrypt_trigger.get(), self.encrypt_mode.get(), self.directory)
...
class DecryptionApp:
    def __init__(self, master):
        ...
        self.decrypt_trigger_menu = tk.OptionMenu(master, self.decrypt_trigger, "Create", "Delete", "Modify", command=self.update_mode)
        ...
        self.decrypt_mode_menu = tk.OptionMenu(master, self.decrypt_mode, "Individual", "Group", "All", command=self.update_mode)
    ...
    def update_mode(self, event=None):
        if hasattr(self, 'handler'):
            self.handler.mode = self.decrypt_mode.get()

    # Method to start monitoring
    def start_monitoring(self):
        if hasattr(self, 'directory') and hasattr(self, 'key_file'):
            self.handler = DecryptionHandler(self.load_key(), self.decrypt_trigger.get(), self.decrypt_mode.get(), self.directory)
